...
struct packet_stats;
class dpdk_thread_info;

class Dpdk {
    friend class dpdk_thread_info;
...
class Connection{
    public:
         NetAddress src_addr;
         NetAddress dest_addr;

        struct rte_mbuf **buf{nullptr};
        struct rte_mbuf* connection_req_pkt;
         int buf_alloc(struct rte_mempool* mbuf_pool);

        void make_headers();
        void make_pkt_header(struct rte_mbuf* pkt);
        void make_connection_request();
        Connection(NetAddress src,  NetAddress dst){
            src_addr = src;
            dest_addr = dst;
